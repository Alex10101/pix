...
const isAnalytics = process.env.ANALYTICS_ENV;
...
const sassResources = [
  ...
  // path.resolve(__dirname, '../src/styles/vars/main.scss')
];
...
module.exports = function(webpackEnv) {
  ...
  const getDevTool = (isEnvProd, sourceMap, isEnvDev) => {
    if (isEnvProd) {
      if (sourceMap) {
        return 'source-map';
      }
    ...
    return isEnvDev && 'cheap-module-source-map';
  };
  ...
  return {
    ...
    devtool      : getDevTool(isEnvProduction, shouldUseSourceMap, isEnvDevelopment), // These are the "entry points" to our application.
    ...
    output       : {
      ...
      publicPath, // Point sourcemap entries to original disk location (format as URL on Windows)

      devtoolModuleFilenameTemplate: isEnvProduction ? (info) => path.relative(paths.appSrc, info.absoluteResourcePath)
      .replace(/\\/g, '/') : isEnvDevelopment && ((info) => path.resolve(info.absoluteResourcePath)
      .replace(/\\/g, '/')), // Prevents conflicts when multiple Webpack runtimes (from different apps)
    ...
    resolve      : {
      ...
      ].concat(modules.additionalModulePaths || []), // These are the reasonable defaults supported by the Node ecosystem.
        ...
        { parser: { requireEnsure: false } },
        {
          ...
          oneOf: [
            ...
            // A missing `test` is equivalent to a match.
            {
              ...
              options: {
                limit: parseInt(process.env.IMAGE_INLINE_SIZE_LIMIT || '10000', 10),
    ...
    plugins      : [
      ...
      new ManifestPlugin({
        ...
        publicPath,
        generate: (seed, files) => {
          const manifestFiles = files.reduce(function(manifestArg, file) {
      ...
      isAnalytics && new BundleAnalyzerPlugin()
